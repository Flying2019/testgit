...
void solve() {
  ...
  for (int x = 1; x <= n; ++x) {
    ...
    for (auto &idx: query[x]) {
      ...
      //printf("ql=%d qr=%d it=(%d %d) cost=%lld\n", ql[idx], qr[idx], it->first, it->second, cost(it->second, ql[idx]));
    }
  }
}
}
  ...
  for (int len = 1; len <= n + 2; len <<= 1) {
    ...
    for (int i = 0; i < n + 2; i += len) {
      ...
      auto dfs = [&](auto self, int l, int r, int xl, int xr) {
        ...
        //printf("xm=%d m=%d l=%d xl=%d xr=%d best=%lld\n", xm, m, ql[ls[m]], xl, xr, best);
...
int main() {
  ...
  prepare();
//   for (int i = 1; i <= n; ++i) printf("%d ",bound[i]);
  hull::solve();
  std::reverse(a, a + n + 2);
  for (int i = 0; i < q; ++i) {
    ql[i] = n + 1 - ql[i];
