### [链接](https://codeforces.com/contest/700/problem/E)
和子串出现次数有关，先建SAM再说。首先贪心可以发现，如果 $s_u\rightarrow s_v$ 符合条件，$s_u$ 一定是 $s_v$ 的前缀和后缀。

可以发现，SAM上如果 $s_u\rightarrow s_v$ 符合条件，显然有 $u$ 是 $v$ 的祖先，而且有对于 $u$ 的祖先 $u'$ 也有 $s_u'\rightarrow s_v$ 符合条件。
