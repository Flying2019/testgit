### [链接](https://codeforces.com/problemset/problem/19/E)
首先分类讨论，可以发现如果只有一个奇环，那么答案就是整个环上的点。

如果不是。首先建出一棵生成树，可以发现每条非树边都表示树上的一条链。

简单推导可以发现，符合要求的是不被偶环包含且被所有奇环包含的边。简单差分即可。
