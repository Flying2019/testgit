### [链接](http://codeforces.com/problemset/problem/528/D)
带模糊处理的字符串匹配问题。显然一般的字符串操作无法支持，考虑FTT。

字符集只有4，直接对每个字符单独处理。

假设字符为 $c$，可以用一些方法求出可以放 $t$ 中可以放 $c$ 的位置。

考虑用 $t$ 的反串去卷，可以发现同一个字符串匹配到的点数被放在了一起。

所以可以匹配当且仅当4种字符的匹配数 $\leq 4$。

显然NTT即可。复杂度 $O(4n\log n)$。
```cpp
#include<iostream>
#include<cstdio>
#include<cstring>
