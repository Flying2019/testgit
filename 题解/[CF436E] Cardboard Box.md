### [链接](http://codeforces.com/problemset/problem/436/E)
考虑假如我们现在要多加一颗星，有什么转移的方式：
1. 把一个没有玩过的关卡玩到一颗星。
2. 把一个玩到一颗星的关卡玩到两颗星。
3. 把一个玩到两颗星的关卡撤销至一颗星，然后把一个没有玩过的关卡玩到两颗星。
4. 把一个玩到一颗星的关卡撤销，然后把一个没有玩过的关卡玩到两颗星。

这样贪心转移显然是对的。分别转移对应的方式，然后找到最大值即可。

具体来说，我们需要用5个堆，分别存：从0到1，从1到2，从2到1，从0到2，从2到1。

这个和模拟费用流的套路类似。复杂度 $O(n\log n)$。
```cpp
#include<iostream>
#include<cstdio>
#include<cstring>
