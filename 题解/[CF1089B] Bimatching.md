### [链接](https://codeforces.com/contest/1089/problem/B)
### 题目大意
给你一个二分图，其中左边一个点必须匹配右边两个点才认为是一组匹配。问最多有几组匹配。
### 题解
显然一对二不能直接跑匹配。考虑拆点。但是直接拆点跑匹配显然是错的，因为这样条件变成“一个左边的点可以匹配两个右边的点”。

所以考虑一种等效：我们在拆开的点中互相连边，即默认这两个点互相匹配。那么显然在增广过程中，要断掉这条边进行增广，势必两个点都要匹配上右边的点。

但是这样得到的图并不是二分图。不过这道题点数很小，可以直接跑一般图最大匹配的带花树算法。
