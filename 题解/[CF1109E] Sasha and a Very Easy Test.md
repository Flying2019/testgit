### [链接](https://codeforces.com/contest/1109/problem/E)
看一眼：这不是神必题吗？直接线段树就好了。

打完之后：我***，这mod不是质数啊。。。

首先根据欧拉定理，$a^{\varphi(p)-1}\equiv 1\pmod p$，所以这样可以方便处理出 1,3 操作。

但是2怎么办？首先肯定不能直接乘逆元，所以考虑在存乘积的时候把不能用逆元求的部分质因子存下来，这样这部分的除法操作相当于对应指数相减，这个直接减完后重新计算一遍即可。

复杂度 $O(\sqrt{mod}+n\log n\log mod)$。
