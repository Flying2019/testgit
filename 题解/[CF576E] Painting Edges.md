### [链接](https://codeforces.com/problemset/problem/576/E)
首先考虑如何只带插入下动态维护图是否为二分图。可以利用并查集，同时维护一个点到根节点的路径长度奇偶性。如果一条边链接了同一连通块的两个点，那么就要求这两个点到根节点的距离奇偶性相同。

但是很明显并查集是不支持删除的，而题目操作中修改边可能会删除并查集中的边。题目没有要求强制在线，所以直接将询问离线，线段树分治。即把每条边的作用范围加到线段树上。

但是题目中一次操作如果是非法的，就不会生效，所以一条边的作用范围是不确定的。但我们可以不妨钦定这条边生效，如果失效就将原边重新作用，显然这不会影响时间复杂度。
