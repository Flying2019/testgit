### [链接](https://codeforces.com/contest/626/problem/E)
三分好题。

考虑枚举中位数，那么根据贪心，偶数长度的结果一定不会比奇数长度的结果优，所以直接考虑奇数。

那么很显然中位数左右两边各一半。根据贪心，我们一定尽可能选最大的。所以考虑选 $[x-l/2,x]$ 和 $(n-l/2,n]$。

这个可以通过前缀和快速计算。又可以发现，随着长度的增大，实际上我们选的数的和的增长速度实在不断变小的。可以证明，它是一个单峰函数。

所以直接三分长度即可。复杂度 $O(n\log n)$。
```cpp
#include<iostream>
#include<cstdio>
#include<cstring>
