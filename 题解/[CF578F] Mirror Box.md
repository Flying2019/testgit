### [链接](https://codeforces.com/contest/578/problem/F)
首先将**点阵**黑白染色。可以发现，一个镜子相当于一次连边。由“经过所有点”可知这张图没有环。由于连边不能交叉，所以这张图的黑白点要么黑点联通，要么白点联通。

显然可得最后点会连成生成树。所以对黑白点单独考虑，利用矩阵树定理，如果一条边已经连上就将两个联通块合并，否则就加入矩阵，将计算结果相加即可。

复杂度看似 $O({(nm)}^3)$，由于题目给定不确定的点最多只有 $k\leq 200$ 个，只会组成 $O(k)$ 个联通块，所以实际是 $O(k^3)$。
```cpp
#include<iostream>
#include<cstdio>
#include<cstring>
