### [链接](http://codeforces.com/problemset/problem/526/G)
考虑一个性质：如果 $x$ 为根，那么答案可以看作是 $y*2-1$ 个叶子到根路径的交。这个可以直接贪心求出。

考虑我们不可能换根求出。可以发现无论 $x,y$ 是多少，都有包含直径的一端。

所以我们不妨以直径为根，预处理所有 $y$ 的对应答案。但是这样有可能最优解不包含 $x$ 。所以我们倍增求出第一个在 $y$ 条路径前已经被访问的点，然后加上这段路径的最大值即可。
