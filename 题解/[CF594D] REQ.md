### [链接](https://codeforces.com/contest/594/problem/D)
考虑 $\varphi(x)$ 的一种计算方法是分解质因式后对于 $p^a$ 次项，对答案贡献为 $(p-1)p^{a-1}$。

那么就有一个很显然的暴力莫队做法：直接用桶记录所有质因子信息，然后暴力处理。复杂度 $O(n\sqrt{n}\log n)$，应该会T。

考虑怎么优化。首先对于一个数 $a$，其大于 $\sqrt{a}$ 的质因子最多只有一个。而1000以内的质数数量不多。所以考虑预处理出这部分。

然后对于大于1000的部分直接用桶存，小于1000的部分在查询时暴力处理。

由于小于1000的质因子只有168个，所以总复杂度 $O(m\sqrt{n}+168m)$，可以通过。
```cpp
#include<iostream>
#include<cstdio>
#include<cstring>
