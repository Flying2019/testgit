### [链接](http://codeforces.com/problemset/problem/601/E)
考虑如果只有一次询问，这就是一个01背包模板题，显然可以在 $O(nm)$ 时间内完成。

但是背包并不支持删除一个元素，按照套路，使用线段树分治。

具体来说，建一颗时间线段树，把每个元素看成一个时间的区间。我们可以在线段树上打标记，然后递归处理。每次回溯时还原数组即可。
