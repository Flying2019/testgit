### [链接](https://codeforces.com/problemset/problem/487/E)
考虑一个性质：如果两个点处于同一个点双联通分量中，那么一定存在一条经过这两个点的路径经过该点双联通分量中权值最小的点。

很明显考虑缩点建圆方树。可以发现，查询就是链最小值，直接缩点上树剖即可。

但是修改有些问题。如果由于一个点可能存在于多个点双联通分量中。这时候如果暴力修改可能直接T掉。

我们考虑使用圆方树的常用技巧，就是作为根的圆点不更新作为儿子的方点，而是方点在查询时顺带更新作为根的圆点的价值。

这样可以发现，修改只需要改它父亲的方点即可。而查询只需要在跳到lca时判断一下，如果结果是方点，那么更新作为父亲的圆点的价值。
