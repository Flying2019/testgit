### [链接](http://codeforces.com/problemset/problem/609/E)
很套路的题目。

考虑我们把最小生成树给建出来。

接下来我们强制加上了一条边，可以发现如果我们直接加上后存在一个环，也就是说接下来我们要割掉环上的一条不是刚加入的边，使得最后和最小。

那么很显然就是割掉最大那条。而且很显然这样也是最优的。

那么这样相当于求链最大值，显然有一个 $O(m\log^2 n)$ 的树剖做法，当然还有 $O(m\log n)$ 的做法，比如ST表优化，建树上主席树，但是不见得比树剖跑得快，而且更难写。
