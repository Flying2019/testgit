### [链接](http://codeforces.com/problemset/problem/524/E)
首先考虑“保护”的性质。可以发现，相当于每个矩形的所有行都有一个车或所有列都有一个车。

显然可以分开考虑。先考虑列的情况。按照套路将所有矩形按y坐标排序。

可以发现，对于每一列，最有用的是最上面的那个车。所以我们对每一列都统计出最上面车的y坐标。

那么查询时我们只要查当前的 $[x_l,x_r]$ 中最上面车的y坐标最小值是否大于 $y_l$ 即可。

对于行的情况同理。时间复杂度 $O(n\log n)$。
```cpp
#include<iostream>
#include<cstdio>
#include<cstring>
