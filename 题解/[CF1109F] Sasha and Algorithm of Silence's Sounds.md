### [链接](https://codeforces.com/problemset/problem/1109/F)
毒瘤题。

首先可以发现，假如我们在相邻的点之间连边，那么它是一棵树当且仅当它只有一个联通块且不存在环。

对于后者，我们可以发现这等价于每一条边都成功合并了两个联通块。而且显然，如果一个点集 $U$ 包含了 $V$，$V$ 中有环，那么 $U$ 中一定有环，这个符合双指针的性质，即我们直接对于每个 $l$，找到最右端的符合条件的右端点。同一个联通块的加删边可以直接用LCT处理。

对于前者，由于它建立在后者的基础上，所以这至少是一个森林，而森林中联通块数目等于点数-边数。这里注意的是虽然边数和点数都有单调性，但是两者之差并没有单调性，不能双指针。

我们可以考虑用线段树处理，即对于每一个区间，处理出它的点数与边数之差的最小值以及最小值的出现次数。如果最小值为1那么出现次数就是答案。直接统计即可。
