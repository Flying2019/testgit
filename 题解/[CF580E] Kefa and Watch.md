### [链接](https://codeforces.com/problemset/problem/580/E)
挺套路的题。

看到循环节，想到Hash。因为 $[l,r]$ 存在长度为 $d$ 的循环节当且仅当子串 $[l,r-d]$ 和子串 $[l+d,r]$ 全等。需要特判 $d=r-l+1$ 的情况。

有修改？没关系，Hash可以套上线段树。由于字典序只有10，暴力统计出每种字符重复 $k$ 次的Hash值，区间覆盖直接打标记即可。
