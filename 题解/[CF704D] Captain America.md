### [链接](http://codeforces.com/problemset/problem/704/D)
首先我们不妨假设有 $r<b$，那么按照套路我们假设所有点染蓝色，那么题目就变成能否尽可能多得将蓝色变成红色。

可以发现对于题目要求的限制，我们可以转换成：要求某一行/列的红点数在 $\left[\ \lceil\frac{cnt+a}{2}\rceil , \lfloor \frac{cnt+a}{2}\rfloor\ \right]$。

考虑行列建点，每个坐标点对应一条边，每个点染红等同于边上流量为1，限制等同于源$\rightarrow$行点限流 $[l,r]$。

直接套用有源汇上下界最大流。按照套路建超级源汇转换成成无源汇上下界可行流，然后再在残留网络上跑一遍普通网络流即可。

特别的，如果某个点上界小于下界，那么直接输无解即可。

图等价于二分图，所以复杂度 $O(m\sqrt{n})$。
```cpp
#include<iostream>
#include<cstdio>
#include<cstring>
